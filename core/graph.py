...
def add_routing(
    ...
    # Router branches: companion handles all research tasks
    # (dispatch pipeline removed — agents now use GrimClient SDK sessions)
    graph.add_conditional_edges(
        "router",
        route_decision,
        {"companion": "companion", "dispatch": "companion"},
    )
...
def build_graph(
    ...
    Composes preprocessing, routing, and postprocessing sections
    into the full graph. Agent execution now uses GrimClient SDK sessions
    (core/client.py) — the graph handles companion paths only.
    ...
    tool_context.configure(mcp_session=mcp_session)
    ...
    # Keyword-based routing: graph_router → [research | personal | planning]
    # All paths go through companion nodes (dispatch pipeline removed)
    ...
    node_count = 11  # identity compress memory skill_match graph_router
                     # router companion personal_companion planning_companion
                     # integrate evolve
